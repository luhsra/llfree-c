...
static inline child_t ll_unused child_new(uint16_t free)
{
	assert(free <= LLFREE_CHILD_SIZE);
	return (child_t){ .free = free, .huge = false };
}
/// Initializes the child entry with the given parameters
static inline child_t ll_unused child_huge(void)
{
	return (child_t){ .free = 0, .huge = true };
}
