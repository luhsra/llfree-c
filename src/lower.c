...
{
	assert(offset % (1 << (MAX_ORDER - HUGE_ORDER)) == 0);
	...
	if (init == INIT_VOLATILE) {
		...
		size_children = align_up(size_children,
					 MAX(TREE_CHILDREN * sizeof(child_t),
					     sizeof(bitfield_t)));
...
void lower_clear(lower_t *self, bool free_all)
{
	...
	for (size_t i = self->childs_len;
	     i < align_up(self->childs_len, TREE_CHILDREN); i++) {
...
result_t lower_recover(lower_t *self)
{
	for (size_t i = 0; i < self->childs_len; ++i) {
		...
		if (child.huge) {
			...
			uint16_t counter =
				CHILD_SIZE - field_count_ones(&self->fields[i]);
...
{
	assert(order == 0 || order == HUGE_ORDER);
...
{
	printf("lower_t {\n");
	for (size_t i = 0; i < align_up(self->childs_len, TREE_CHILDREN); i++) {
		if (i % TREE_CHILDREN == 0)
			printf("\n");
			  ...
			  void f(void *, uint64_t, uint64_t))
{
	for (uint64_t i = 0; i < self->childs_len; ++i) {
		child_t child = atom_load(&self->children[i]);
		f(context, self->offset + i * (1 << HUGE_ORDER), child.free);
	}
}
