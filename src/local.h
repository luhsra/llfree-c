...
/// Size of the local metadata for the given clustering.
size_t ll_local_size(const llfree_clustering_t *clustering);

/// Initialize the per-cpu data.
/// The clusters array will contain slices pointing into the metadata buffer.
...
/// Returns the number of local slots for a given cluster, or ll_none() if cluster not configured.
...
typedef struct local_result {
	...
	uint8_t cluster; /// cluster of the reservation
	treeF_t free; /// free count
	row_id_t start_row; /// bitfield row index of reserved tree
} local_result_t;

static inline ll_unused local_result_t local_ok(uint8_t cluster, treeF_t free,
						row_id_t start_row)
{
...
static inline ll_unused local_result_t local_fail(uint8_t cluster, bool present,
						  treeF_t free,
						  row_id_t start_row)
{
...
/// Decrement the number of free frames for the given (cluster, index).
...
/// Increment the number of free frames for the given (cluster, index).
bool ll_local_put(local_t *self, uint8_t cluster, size_t index, tree_id_t tree_idx,
		  treeF_t frames);

/// Update the starting row for the given (cluster, index).
local_result_t ll_local_set_start(local_t *self, uint8_t cluster, size_t index,
				  row_id_t start_row);

/// Swap (cluster, index) with a new tree (returns the old reservation).
...
/// On success, result.{cluster, free, start_row} describe the stolen slot.
...
typedef struct demote_any_result {
	...
	row_id_t unres_row;
	uint8_t unres_cluster;
	treeF_t unres_free;
} demote_any_result_t;

/// Demote from another cluster's local slot
/// Finds a target slot where policy(cluster, target_cluster, frames) == DEMOTE,
