...
#define check_child_number(expect)                     \
	check_uequal_m(actual.num_of_childs, (expect), \
		       "schould have exactly 1 Child per 512 Frames");
...
#define bitfield_is_blocked(actual)                                         \
	check_equal_bitfield(actual,                                        \
			     ((bitfield_t){ u64MAX, u64MAX, u64MAX, u64MAX, \
					    u64MAX, u64MAX, u64MAX, u64MAX }))
...
bool init_lower_test(uint8_t init)
{
	...
	bitfield_is_free(actual.fields[0]) check_uequal(
		lower_allocated_frames(&actual), 0ul) if (init == VOLATILE)
	{
		free_lower(actual);
	}
	...
	check_child_number(2ul);
	check_uequal(lower_allocated_frames(&actual),
		     init == VOLATILE ? 632ul : 631ul) if (init == VOLATILE)
	{
		free_lower(actual);
	}
	...
	lower_init_default(&actual, pfn_start, frames, init);
	ret = lower_init(&actual, true);
	check(((uint64_t)&actual.fields[actual.num_of_childs] -
	       (uint64_t)&actual.fields[0]) /
			      CACHESIZE ==
		      actual.num_of_childs,
	      "no padding");
	check_child_number(1339ul);
	bitfield_is_free_n(actual.fields, 1338) check_equal_bitfield(
		actual.fields[1338],
		((bitfield_t){ 0x0,
			       init == VOLATILE ? 0xfffffe0000000000 :
						  0xfffffffffff80000,
			       u64MAX, u64MAX, u64MAX, u64MAX, u64MAX,
			       u64MAX }))
...
bool get_test()
{
	...
	check_equal_bitfield(actual.fields[0],
			     ((bitfield_t){ u64MAX, u64MAX, u64MAX, u64MAX,
					    u64MAX, u64MAX, u64MAX, u64MAX }))
		check_equal_bitfield(actual.fields[1],
				     ((bitfield_t){ u64MAX, u64MAX, u64MAX,
	...
	check_equal(ret, 0);
	check_equal_bitfield(actual.fields[0],
			     ((bitfield_t){ 0xfffffffffffffffd, u64MAX, u64MAX,
					    u64MAX, u64MAX, u64MAX, u64MAX,
					    u64MAX }))
...
bool put_test()
{
	...
	check_equal(ret, ERR_OK) check_equal_bitfield(
		actual.fields[0],
		((bitfield_t){ 0xfffffffffffffffe, u64MAX, u64MAX, u64MAX,
			       u64MAX, u64MAX, u64MAX, u64MAX }))
		check_equal_bitfield(actual.fields[1],
				     ((bitfield_t){ u64MAX, u64MAX, u64MAX,
						    u64MAX, u64MAX, u64MAX,
		...
		pfn = 0;
	ret = lower_put(&actual, pfn, order);
	check_equal(ret, ERR_ADDRESS) check_equal_bitfield(
		actual.fields[0],
		((bitfield_t){ 0xfffffffffffffffe, u64MAX, u64MAX, u64MAX,
			       u64MAX, u64MAX, u64MAX, u64MAX }))
		check_equal_bitfield(actual.fields[1],
	...
	check_equal(ret, ERR_OK) check_equal_bitfield(
		...
		((bitfield_t){ 0xfffffffffffffffe, u64MAX, u64MAX, u64MAX,
			       u64MAX, u64MAX, u64MAX, u64MAX }))
		check_equal_bitfield(actual.fields[1],
				     ((bitfield_t){ 0xfffdffffffffffff, u64MAX,
						    u64MAX, u64MAX, u64MAX,
						    u64MAX, 0x1fffffffffffffff,
						    0x0 }))
...
int lower_HP_tests()
{
	...
	for (int i = 3; i < 60; ++i) {
		...
		int64_t pfn = lower_get(&actual, i < 32 ? 0 : 32 * FIELDSIZE,
					HP_ORDER);
		check(pfn > 0, "");
	}
...
bool persistent_init()
{
	...
	check((uint64_t)&lower.childs[lower.num_of_childs] <
		      lower.start_frame_adr + len * PAGESIZE,
	      "");
	check((uint64_t)&lower.childs[lower.num_of_childs] >
		      lower.start_frame_adr,
	      "");
	check((uint64_t)&lower.fields[lower.num_of_childs] <
		      lower.start_frame_adr + len * PAGESIZE,
	      "");
	check((uint64_t)&lower.fields[lower.num_of_childs] >
		      lower.start_frame_adr,
...
int lower_tests(int *test_counter, int *fail_counter)
{
	...
	run_test(init_persistent_test) run_test(get_test);
	run_test(put_test);
